...
description: Record a transaction using the shared DeepLedger procedure for tool selection, duplicate checks, approval, recording and verification. Use when the user asks to record, enter, book or log a payment, purchase, sale, refund or deposit.
---

# Record Transactions

Record the requested transaction with the correct tool, required approval and a verified outcome.
...
```text
getGuide(guideType="transaction_recording")
```
